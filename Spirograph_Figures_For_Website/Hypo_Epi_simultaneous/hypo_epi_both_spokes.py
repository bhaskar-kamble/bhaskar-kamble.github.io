...
lines.append(lobj)
#weiter:---------------------
lobj = ax.plot([],[],lw=1,color="black")[0] # curve 2
lines.append(lobj)
...
theta = np.linspace(0.0,-2.0*mr1*pi,int(mr1*200.0))
...
theta2 = np.linspace(0,2.0*mr2*pi,int(mr1*200.0))
...
def animate(i):
    ...
    for lnum,line in enumerate(lines):
        ...
        if lnum==15:
            line.set_data(xd2, yd2) # 2nd curve [:i] for curve till now
        if lnum==16:
...
anim.save("hypo_r_2by7_5by7_both.gif",dpi=80,writer='imagemagick')#, fps=30)
