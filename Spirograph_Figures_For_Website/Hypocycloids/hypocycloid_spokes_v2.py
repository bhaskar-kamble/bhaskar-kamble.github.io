...
lines.append(lobj)
lobj = ax.plot([],[],lw=2,color="black")[0]  #the hypocycloid
lines.append(lobj)
...
theta = np.linspace(0,2.0*mr*pi,int(mr)*100)
...
def animate(i):
    ...
    for lnum,line in enumerate(lines):
        ...
        if lnum==1:
            line.set_data(xd, yd) #hypocycloid [:i] for path traced till now
        if lnum==2:                                       
...
anim.save("hypo_r_1by2_spokes.gif",dpi=80,writer='imagemagick')
